...
{{ cookiecutter.project_slug }} aims to be out-the-box compatible with
...
that PyInstaller does need special help when compiling {{ cookiecutter.project_slug }}
then that *help* goes in a *hook* file (see
`{{ cookiecutter.project_slug }}/__pyinstaller/hook-{{ cookiecutter.project_slug }}.py`).
...
changed the {{ cookiecutter.project_slug }} source code.
...
This executable is just `pytest.main()` with {{ cookiecutter.project_slug }}
...
Otherwise you may unintentionally use the original source {{ cookiecutter.project_slug }}
...
If you've modified either the source code or the hooks for {{ cookiecutter.project_slug }}
...
`{{ cookiecutter.project_slug }}/__pyinstaller/`).
